...
public class client extends RSApplet {
	...
	public void drawChannelButtons() {
		...
		smallText.method389(true, 77, 0xffffff, "Game", 158);
		...
		smallText.method389(true, 363, 0xffffff, "Duel", 153);
		/* Chat modes */
		...
		smallText.method382(textColor[clanChatMode], 260, text[clanChatMode], 164, true);
		smallText.method382(textColor[tradeMode], 317, text[tradeMode], 164, true);
		smallText.method382(textColor[duelMode], 374, text[duelMode], 164, true);
	}
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
	...
	private void processChatModeClick() {
		...
		else if(super.mouseX >= 347 && super.mouseX <= 403 && super.mouseY >= 482 && super.mouseY <= 505) {
			cButtonHPos = 6;
			aBoolean1233 = true;
			inputTaken = true;
		}
		...
		if(super.clickMode3 == 1) {
			...
			if(super.saveClickX >= 347 && super.saveClickX <= 403 && super.saveClickY >= 482 && super.saveClickY <= 505) {
				cButtonCPos = 6;
				chatTypeView = 4;
				aBoolean1233 = true;
				inputTaken = true;
	...
	public void drawSideIcons(){
		/* Top sideIcons */
		if(tabInterfaceIDs[0] != -1 && anInt1054 == 0)
			if(loopCycle % 20 >= 10);
				sideIcons[0].drawSprite(10, 4);
		if(tabInterfaceIDs[1] != -1 && anInt1054 == 1)
			if(loopCycle % 20 >= 10);
				sideIcons[1].drawSprite(43, 4);
		if(tabInterfaceIDs[2] != -1 && anInt1054 == 2)
			if(loopCycle % 20 >= 10);
				sideIcons[2].drawSprite(76, 3);
		if(tabInterfaceIDs[3] != -1 && anInt1054 == 3)
			if(loopCycle % 20 >= 10);
				sideIcons[3].drawSprite(111, 5);
		if(tabInterfaceIDs[4] != -1 && anInt1054 == 4)
			if(loopCycle % 20 >= 10);
				sideIcons[4].drawSprite(140, 1);
		if(tabInterfaceIDs[5] != -1 && anInt1054 == 5)
			if(loopCycle % 20 >= 10);
				sideIcons[5].drawSprite(174, 1);
		if(tabInterfaceIDs[6] != -1 && anInt1054 == 6)
			if(loopCycle % 20 >= 10);
				sideIcons[6].drawSprite(209, 5);
		/* Bottom sideIcons */
		if(tabInterfaceIDs[7] != -1 && anInt1054 == 7)
			if(loopCycle % 20 >= 10);
				sideIcons[7].drawSprite(43, 302);
		if(tabInterfaceIDs[8] != -1 && anInt1054 == 8)
			if(loopCycle % 20 >= 10);
				sideIcons[8].drawSprite(76, 302);
		if(tabInterfaceIDs[9] != -1 && anInt1054 == 9)
			if(loopCycle % 20 >= 10);
				sideIcons[13].drawSprite(112, 304);
		if(tabInterfaceIDs[10] != -1 && anInt1054 == 10)
			if(loopCycle % 20 >= 10);
				sideIcons[10].drawSprite(143, 303);
		if(tabInterfaceIDs[11] != -1 && anInt1054 == 11)
			if(loopCycle % 20 >= 10);
				sideIcons[11].drawSprite(173, 300);
		if(tabInterfaceIDs[12] != -1 && anInt1054 == 12)
			if(loopCycle % 20 >= 10);
				sideIcons[12].drawSprite(206, 300);
	}
 ...
 //	   myPassword = "";
  ...
  private static boolean repackCRCs = false; // Flags to re-pack CRC values after update (developer only)
	public static void main(String args[]) {
		try {
			System.out.println("RS2 user client - release #" + 317);
			System.out.println(args.length);
			if(args.length == 1) {
				if (args[0].toLowerCase().equals("repackcrcs")) {
          repackCRCs = true;
          System.out.println("Developer: repacking CRCs ON");
        }
			}
			nodeID = 10;
			portOff = 0;
	...
	private void processTabClick() {
		if(super.clickMode3 == 1) {
			...
			if(super.saveClickX >= 742 && super.saveClickX <= 764 && super.saveClickY >= 1 && super.saveClickY < 24 && tabInterfaceIDs[10] != -1)
			{
				needDrawTabArea = true;
				tabID = 10;
				tabAreaAltered = true;
	...
	private void rightClickChatButtons() {
		...
		else if(super.mouseX >= 347 && super.mouseX <= 403 && super.mouseY >= 482 && super.mouseY <= 503) {
 ...
 //		   return;
      ...
      connectServer();
        ...
        for (int i = 0; i < decompressors.length; i++) {
          onDemandFetcher.crcPack(i, (int) decompressors[i].getFileCount());
          System.out.println("Repacked CRCs - decompressor: "+i);
        }
      }
			...
			chatButtons = new Sprite("chatbuttons");
	...
	public void drawHPOrb() {
		...
		/* Draws empty orb */
		emptyOrb.drawSprite(170, 13);
		/* Draws current HP text */
		if(health <= 100 && health >= 75) {
			smallText.method382(65280, 213, cHP, 39, true);
		}
		else if(health <= 74 && health >= 50) {
			smallText.method382(0xffff00, 213, cHP, 39, true);
		}
		else if(health <= 49 && health >= 25) {
			smallText.method382(0xfca607, 213, cHP, 39, true);
		}
		else if(health <= 24 && health >= 0) {
			smallText.method382(0xf50d0d, 213, cHP, 39, true);
		}
	...
	public void drawPrayerOrb() {
		...
		String cP = RSInterface.interfaceCache[4012].message; cP = cP.replaceAll("%","");
			int currentPrayer = Integer.parseInt(cP);
		String mP = RSInterface.interfaceCache[4013].message; mP = mP.replaceAll("%","");
			int maxPrayer = Integer.parseInt(mP);
		prayer = (int)(((double)currentPrayer / (double)maxPrayer) * 100D);
		/* Draws empty orb */
		emptyOrb.drawSprite(184, 49);
		/* Draws current HP text */
		if(prayer <= 100 && prayer >= 75) {
			smallText.method382(65280, 227, cP, 75, true);
		}
		else if(prayer <= 74 && prayer >= 50) {
			smallText.method382(0xffff00, 227, cP, 75, true);
		}
		else if(prayer <= 49 && prayer >= 25) {
			smallText.method382(0xfca607, 227, cP, 75, true);
		}
		else if(prayer <= 24 && prayer >= 0) {
			smallText.method382(0xf50d0d, 227, cP, 75, true);
		}
		/* Draws inside orb sprites */
	...
	public void drawLogoutButton() {
		if(tabInterfaceIDs[9] != -1 && anInt1054 == 9)
			if(loopCycle % 20 >= 10);
				sideIcons[9].drawSprite(223, 1);
		if(logIconHPos == 1) {
			logIconH.drawSprite(223, 1);
			if(tabID == 10)
...
//			}
	...
	public static String server = "furbyhead.no-ip.biz";
